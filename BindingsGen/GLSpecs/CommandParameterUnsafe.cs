...
namespace BindingsGen.GLSpecs
{
    ...
    class CommandParameterUnsafe : CommandParameterStrong
    {
            ...
            : base(otherParam, ctx, parentCommand)
        {
            ...
            _IsPointer = IsCompatible(parentCommand, otherParam);
        }
        ...
        internal static bool IsCompatible(Command command)
        {
            return IsCompatible(command, command.Parameters);
        }

        internal static bool IsCompatible(Command command, List<CommandParameter> parameters)
        {
            return parameters.FindIndex(delegate (CommandParameter item) { return IsCompatible(command, item); }) >= 0;
        }

        internal static bool IsCompatible(Command command, CommandParameter param)
        {
