...
namespace OpenGL
{
	...
	/// </summary>
	public class MatrixDouble3x3 : MatrixDouble, IMatrix3x3
	{
		#region Constructors
		...
		/// </exception>
		public MatrixDouble3x3(double[] values)
			: base(values, 3, 3)
		{
		...
		/// </summary>
		/// <param name="m">
		/// A <see cref="MatrixDouble3x3"/> to be copied.
		/// </param>
		/// <exception cref="ArgumentNullException">
		/// Exception throw if <paramref name="m"/> is null.
		/// </exception>
		public MatrixDouble3x3(MatrixDouble3x3 m)
			: base(m)
		{
		...
		/// </exception>
		public static Vertex3d operator *(MatrixDouble3x3 m, Vertex3d v)
		{
			if (m == null)
		...
		/// Exception thrown if <paramref name="m"/> is null.
		/// </exception>
		public static Vertex4d operator *(MatrixDouble3x3 m, Vertex4d v)
		{
			if (m == null)
				throw new ArgumentNullException("m");

			return ((Vertex4d)(m * (Vertex3d) v));
		}
		...
		/// </exception>
		public static MatrixDouble3x3 operator *(MatrixDouble3x3 m1, MatrixDouble3x3 m2)
		{
			// Allocate product matrix
			MatrixDouble3x3 prod = new MatrixDouble3x3();
		...
		/// </exception>
		private static void ComputeMatrixProduct(MatrixDouble3x3 result, MatrixDouble3x3 m, MatrixDouble3x3 n)
		{
			if (result == null)
		...
		/// </summary>
		/// <param name="a">
		/// A <see cref="Matrix3x3"/> to be converted to <see cref="MatrixDouble3x3"/>.
		/// </param>
		/// <returns>
		/// A <see cref="MatrixDouble3x3"/> equivalent to <paramref name="a"/>.
		/// </returns>
		public static implicit operator MatrixDouble3x3(Matrix3x3 a)
		{
			if (a == null)
				return (null);
			
			MatrixDouble3x3 matrix = new MatrixDouble3x3();
		...
		/// </summary>
		/// <returns>
		/// A <see cref="MatrixDouble3x3"/> which hold the transpose of this Matrix3x3.
		/// </returns>
		public new MatrixDouble3x3 Transpose()
		{
			MatrixDouble3x3 t = new MatrixDouble3x3();
		...
		{
			return (new MatrixDouble3x3(this));
		}
		...
		/// </exception>
		public new MatrixDouble3x3 GetInverseMatrix()
		{
			//        | a b c |
			// m    = | d e f |
			//        | g h k |
			...
			MatrixDouble3x3 inverse = new MatrixDouble3x3();
			double inv = 1.0 / determinant;

			unsafe {
				fixed (double* m = MatrixBuffer) {
				fixed (double* mi = inverse.MatrixBuffer) {
						double a = m[0], b = m[3], c = m[6];
						double d = m[1], e = m[4], f = m[7];
						double g = m[2], h = m[5], k = m[8];
