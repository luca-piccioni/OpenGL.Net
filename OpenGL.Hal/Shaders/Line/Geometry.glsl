...
uniform float hal_LineWidth = 1.0;

// Processed vertex color
SHADER_IN vec4 hal_VertexColor[2];

// Processed primitive vertex color
SHADER_OUT vec4 hal_GeoColor;
...
void main()
{
	...
	vec2 lineScreenOffset = (vec2(hal_LineWidth) / hal_ViewportSize) * lineScreenRight;

	gl_Position = vec4(ndc0.xy + lineScreenOffset, ndc0.z, 1.0);
	hal_GeoColor = hal_VertexColor[0];
	EmitVertex();
	...
	hal_GeoColor = hal_VertexColor[1];
	EmitVertex();
