...
namespace OpenGL
{
	public partial class Gl
	{
		...
		/// for the compute shader stage. While the individual shader invocations within a work group are executed as a unit, work 
		/// groups are executed completely independently and in unspecified order. num_groups_x, num_groups_y and num_groups_z 
		...
		/// currently bound to the GL_DISPATCH_INDIRECT_BUFFER target. Each work group is processed by the active program object for 
		/// the compute shader stage. While the individual shader invocations within a work group are executed as a unit, work 
		/// groups are executed completely independently and in unspecified order. indirect contains the offset into the data store 
		/// of the buffer object bound to the GL_DISPATCH_INDIRECT_BUFFER target at which the parameters are stored.
		...
		/// such circumstances, behavior is undefined and may lead to application termination.
		/// <para>
		/// The following errors can be generated:
		/// - GL_INVALID_OPERATION is generated if there is no active program for the compute shader stage.
		...
		/// does not perform general-purpose conversions such as scaling, resizing, blending, color-space, or format conversions. It 
		/// should be considered to operate in a manner similar to a CPU memcpy. CopyImageSubData can copy between images with 
		/// different internal formats, provided the formats are compatible.
		/// glCopyImageSubData also allows copying between certain types of compressed and uncompressed internal formats. This copy 
		/// does not perform on-the-fly compression or decompression. When copying from an uncompressed internal format to a 
		/// compressed internal format, each texel of uncompressed data becomes a single block of compressed data. When copying from 
		/// a compressed internal format to an uncompressed internal format, a block of compressed data becomes a single texel of 
		/// uncompressed data. The texel size of the uncompressed format must be the same size the block size of the compressed 
		...
		/// detail in the texture object. For renderbuffers, this value must be zero.
		/// srcX, srcY, and srcZ specify the lower left texel coordinates of a srcWidth-wide by srcHeight-high by srcDepth-deep 
		/// rectangular subregion of the source texel array. Similarly, dstX, dstY and dstZ specify the coordinates of a subregion 
		/// of the destination texel array. The source and destination subregions must be contained entirely within the specified 
		/// level of the corresponding image objects.
		...
		/// will be a factor of the block size larger than in the uncompressed image.
		/// Slices of a GL_TEXTURE_1D_ARRAY, GL_TEXTURE_2D_ARRAY, GL_TEXTURE_CUBE_MAP_ARRAYGL_TEXTURE_3D and faces of 
		/// GL_TEXTURE_CUBE_MAP are all compatible provided they share a compatible internal format, and multiple slices or faces 
		/// may be copied between these objects with a single call by specifying the starting slice with srcZ and dstZ, and the 
		/// number of slices to be copied with srcDepth. Cubemap textures always have six faces which are selected by a zero-based 
		/// face index.
		/// For the purposes of CopyImageSubData, two internal formats are considered compatible if any of the following conditions 
		/// are met: the formats are the same, the formats are considered compatible according to the compatibility rules used for 
		/// texture views as defined in section 3.9.X. In particular, if both internal formats are listed in the same entry of Table 
		/// 3.X.2, they are considered compatible, or one format is compressed and the other is uncompressed and Table 4.X.1 lists 
		/// the two formats in the same row. If the formats are not compatible, an INVALID_OPERATION error is generated.
		/// <para>
		/// The following errors can be generated:
		...
		///   than GL_TEXTURE_BUFFER, or is one of the cubemap face selectors.
		...
		/// being invalidated. After this command, data in that subregion have undefined values. xoffset, yoffset, zoffset, width, 
		/// height, and depth are interpreted as they are in glTexSubImage3D. For texture targets that don't have certain 
		/// dimensions, this command treats those dimensions as having a size of 1. For example, to invalidate a portion of a two- 
		/// dimensional texture, the application would use zoffset equal to zero and depth equal to one. Cube map textures are 
		/// treated as an array of six slices in the z-dimension, where a value of zoffset is interpreted as specifying face 
		...
		/// of the image, respectively. Furthermore, xoffset + width, yoffset + height, and zoffset + depth must be less than or 
		/// equal to the width, height or depth of the image, respectively.
		...
		/// content of the buffer's data store becomes undefined.
		/// <para>
		/// The following errors can be generated:
		/// - GL_INVALID_VALUE is generated if buffer is not the name of an existing buffer object.
		/// - GL_INVALID_OPERATION is generated if any part of buffer is currently mapped.
		...
		/// glMultiDrawArraysIndirect behaves similarly to a multitude of calls to glDrawArraysInstancedBaseInstance, execept that 
		/// the parameters to each call to glDrawArraysInstancedBaseInstance are stored in an array in memory at the address given 
		/// by indirect, separated by the stride, in basic machine units, specified by stride. If stride is zero, then the array is 
		...
		/// is interpreted as an offset, in basic machine units, into that buffer and the parameter data is read from the buffer 
		/// rather than from client memory.
		...
		/// execpt that the parameters to glDrawElementsInstancedBaseVertexBaseInstance are stored in an array in memory at the 
		/// address given by indirect, separated by the stride, in basic machine units, specified by stride. If stride is zero, then 
		...
		/// name of which is given by pname.
		...
		/// resource in programInterface. This length includes an extra character for the null terminator. If the list of active 
		/// resources for programInterface is empty, zero is returned. It is an error to specify GL_MAX_NAME_LENGTH when 
		/// programInterface is GL_ATOMIC_COUNTER_BUFFER, as active atomic counter buffer resources are not assigned name strings.
		/// If pname is GL_MAX_NUM_ACTIVE_VARIABLES, the value returned is the number of active variables belonging to the interface 
		/// block or atomic counter buffer resource in programInterface with the most active variables. If the list of active 
		/// resources for programInterface is empty, zero is returned. When pname is GL_MAX_NUM_ACTIVE_VARIABLES, programInterface 
		...
		/// the active subroutine uniform in programInterface with the most compatible subroutines. If the list of active resources 
		/// for programInterface is empty, zero is returned. When pname is GL_MAX_NUM_COMPATIBLE_SUBROUTINES, programInterface must 
		/// be one of GL_VERTEX_SUBROUTINE_UNIFORM, GL_TESS_CONTROL_SUBROUTINE_UNIFORM, GL_TESS_EVALUATION_SUBROUTINE_UNIFORM, 
		...
		///   including process termination may occur.
		/// </para>
		/// <para>
		/// The associated information is got with the following commands:
		...
		/// which contains the resource named nameand must be one of the following values:
		/// If name exactly matches the name string of one of the active resources for programInterface, the index of the matched 
		/// resource is returned. Additionally, if name would exactly match the name string of an active resource if "[0]" were 
		/// appended to name, the index of the matched resource is returned. Otherwise, name is considered not to be the name of an 
		/// active resource, and GL_INVALID_INDEX is returned.
		/// For the interface GL_TRANSFORM_FEEDBACK_VARYING, the value GL_INVALID_INDEX should be returned when querying the index 
		/// assigned to the special names gl_NextBuffer, gl_SkipComponents1, gl_SkipComponents2, gl_SkipComponents3, or 
		/// gl_SkipComponents4.
		/// <para>
		/// The following errors can be generated:
		...
		///   identified by programInterface.
		/// </para>
		/// </remarks>
		...
		/// interface programInterface of program object program. index must be less than the number of entries in the active 
		...
		/// character array whose address is given in name. The actual number of characters written into name, excluding the null 
		/// terminator, is returned in length. If length is NULL, no length is returned. The maximum number of characters that may 
		/// be written into name, including the null terminator, is specified by bufSize. If the length of the name string including 
		/// the null terminator is greater than bufSize, the first bufSize-1 characters of the name string will be written to name, 
		/// followed by a null terminator. If bufSize is zero, no error will be generated but no characters will be written to name. 
		/// The length of the longest name string for programInterface&gt;, including a null terminator, can be queried by calling 
		...
		/// interface programInterface of program object program. For each resource, values for propCount properties specified by 
		/// the array props are returned. propCount may not be zero. An error is generated if any value in props is not one of the 
		/// properties described immediately belowor if any value in props is not allowed for programInterface. The set of allowed 
		...
		/// returned is one of the values found in table 2.16.
		/// For the property GL_ARRAY_SIZE, a single integer identifying the number of active array elements of an active variable 
		/// is written to params. The array size returned is in units of the type associated with the property GL_TYPE. For active 
		/// variables not corresponding to an array of basic types, the value zero is written to params.
		/// For the property GL_BLOCK_INDEX, a single integer identifying the index of the active interface block containing an 
		/// active variable is written to params. If the variable is not the member of an interface block, the value -1 is written 
		/// to params.
		/// For the property GL_OFFSET, a single integer identifying the offset of an active variable is written to params. For 
		/// variables in the GL_UNIFORM and GL_BUFFER_VARIABLE interfaces that are backed by a buffer object, the value written is 
		/// the offset of that variable relative to the base of the buffer range holding its value. For variables in the 
		/// GL_TRANSFORM_FEEDBACK_VARYING interface, the value written is the offset in the transform feedback buffer storage 
		/// assigned to each vertex captured in transform feedback mode where the value of the variable will be stored. Such offsets 
		/// are specified via the xfb_offset layout qualifier or assigned according to the variables position in the list of strings 
		/// passed to glTransformFeedbackVaryings. Offsets are expressed in basic machine units. For all variables not recorded in 
		/// transform feedback mode, including the special names "gl_NextBuffer", "gl_SkipComponents1", "gl_SkipComponents2", 
		...
		/// is written to params. For active variables declared as an array of basic types, the value written is the difference, in 
		/// basic machine units, between the offsets of consecutive elements in an array. For active variables not declared as an 
		/// array of basic types, zero is written to params. For active variables not backed by a buffer object, -1 is written to 
		...
		/// rows of a row-major matrix is written to params. For active variables declared a single matrix or array of matrices, the 
		/// value written is the difference, in basic machine units, between the offsets of consecutive columns or rows in each 
		...
		/// written to params. For active variables backed by a buffer object, declared as a single matrix or array of matrices, and 
		/// stored in row-major order, one is written to params. For all other active variables, zero is written to params.
		/// For the property GL_ATOMIC_COUNTER_BUFFER_INDEX, a single integer identifying the index of the active atomic counter 
		/// buffer containing an active variable is written to params. If the variable is not an atomic counter uniform, the value 
		...
		/// machine units, required to hold all active variables associated with an active uniform block, shader storage block, or 
		/// atomic counter buffer is written to params. If the final member of an active shader storage block is array with no 
		...
		/// storage block, atomic counter buffer or transform feedback buffer is written to params.
		/// For the property GL_ACTIVE_VARIABLES, an array of active variable indices associated with an active uniform block, 
		/// shader storage block, atomic counter buffer or transform feedback buffer is written to params. The number of values 
		/// written to params for an active resource is given by the value of the property GL_NUM_ACTIVE_VARIABLES for the resource.
		...
		/// referenced by the vertex, tessellation control, tessellation evaluation, geometry, or fragment shaders, respectively, in 
		/// the program object. The value one is written to params if an active variable is referenced by the corresponding shader, 
		/// or if an active uniform block, shader storage block, or atomic counter buffer contains at least one variable referenced 
		...
		/// top-level shader storage block member containing to the active variable is written to params. If the top-level block 
		/// member is not declared as an array, the value one is written to params. If the top-level block member is an array with 
		/// no declared size, the value zero is written to params.
		/// For the property GL_TOP_LEVEL_ARRAY_STRIDE, a single integer identifying the stride between array elements of the 
		/// top-level shader storage block member containing the active variable is written to params. For top-level block members 
		/// declared as arrays, the value written is the difference, in basic machine units, between the offsets of the active 
		/// variable for consecutive elements in the top-level array. For top-level block members not declared as an array, zero is 
		/// written to params.
		/// For the property GL_LOCATION, a single integer identifying the assigned location for an active uniform, input, output, 
		/// or subroutine uniform variable is written to params. For input, output, or uniform variables with locations specified by 
		/// a layout qualifier, the specified location is used. For vertex shader input or fragment shader output variables without 
		/// a layout qualifier, the location assigned when a program is linked is written to params. For all other input and output 
		...
		/// output variable is written to params. If the active variable is an output for a non-fragment shader, the value -1 will 
		...
		/// the active variable is a per-patch attribute (declared with the patch qualifier), the value one is written to params; 
		...
		/// active input or output variable is written to params. For input and output variables with a component specified by a 
		/// layout qualifier, the specified component is written. For all other input and output variables, the value zero is 
		/// written.
		/// For the property GL_TRANSFORM_FEEDBACK_BUFFER_INDEX, a single integer identifying the index of the active transform 
		/// feedback buffer associated with an active variable is written to params. For variables corresponding to the special 
		/// names "gl_NextBuffer", "gl_SkipComponents1", "gl_SkipComponents2", "gl_SkipComponents3", and "gl_SkipComponents4", -1 is 
		/// written to params.
		/// For the property GL_TRANSFORM_FEEDBACK_BUFFER_STRIDE, a single integer identifying the stride, in basic machine units, 
		/// between consecutive vertices written to the transform feedback buffer is written to params.
		/// <para>
		/// The following errors can be generated:
		...
		/// be one of GL_UNIFORM, GL_PROGRAM_INPUT, GL_PROGRAM_OUTPUT, GL_VERTEX_SUBROUTINE_UNIFORM, 
		/// GL_TESS_CONTROL_SUBROUTINE_UNIFORM, GL_TESS_EVALUATION_SUBROUTINE_UNIFORM, GL_GEOMETRY_SUBROUTINE_UNIFORM, 
		...
		/// if name identifies an active variable that does not have a valid location assigned, as described above. The locations 
		/// returned by these commands are the same locations returned when querying the GL_LOCATION and GL_LOCATION_INDEX resource 
		/// properties.
		...
		/// variable, glGetProgramResourceLocation returns the location assigned to that element. If it specifies the base name of 
		/// an array, it identifies the resources associated with the first element of the array.
		/// <para>
		/// The following errors can be generated:
		/// - GL_INVALID_VALUE is generated if program is not the name of an existing program object.
		/// - GL_INVALID_ENUM is generated if programInterface is not one of the accepted interface types.
		...
		/// programInterface of program object program. program must be the name of a program that has been linked successfully. 
		...
		/// program object program. storageBlockIndex must be an active shader storage block index in program. storageBlockBinding 
		/// must be less than the value of GL_MAX_SHADER_STORAGE_BUFFER_BINDINGS. If successful, glShaderStorageBinding specifies 
		/// that program will use the data store of the buffer object bound to the binding point storageBlockBinding to read and 
		...
		/// parent texture's data store with the initialized texture. texture specifies a name previously reserved by a successful 
		/// call to glGenTextures but that has not yet been bound or given a target. target specifies the target for the newly 
		/// initialized texture and must be compatible with the target of the parent texture, given in origtexture as specified in 
		/// the following table:
		/// The value of GL_TEXTURE_IMMUTABLE_FORMAT for origtexture must be GL_TRUE. After initialization, texture inherits the 
		/// data store of the parent texture, origtexture and is usable as a normal texture object with target target. Data in the 
		/// shared store is reinterpreted with the new internal format specified by internalformat. internalformat must be 
		/// compatible with the internal format of the parent texture as specified in the following table:
		/// If the original texture is an array or has multiple mipmap levels, the parameters minlayer, numlayers, minlevel, and 
		/// numlevels control which of those slices and levels are considered part of the texture. The minlevel and minlayer 
		/// parameters are relative to the view of the original texture. If numlayers or numlevels extend beyond the original 
		...
		/// target is GL_TEXTURE_CUBE_MAP_ARRAY, then numlayers counts layer-faces rather than layers, and the clamped numlayers 
		/// must be a multiple of 6. If the new texture's target is GL_TEXTURE_CUBE_MAP or GL_TEXTURE_CUBE_MAP_ARRAY, the width and 
		...
		/// it were a GL_TEXTURE_CUBE_MAP_ARRAY with 6 layer-faces.
		...
		/// For example, if the original texture has a GL_TEXTURE_2D_ARRAY target and its width is greater than 
		/// GL_MAX_CUBE_MAP_TEXTURE_SIZE, an error will be generated if glTextureView is called to create a GL_TEXTURE_CUBE_MAP 
		/// view.
		/// Texture commands that take a level or layer parameter, such as glTexSubImage2D, interpret that parameter to be relative 
		/// to the view of the texture. i.e. the mipmap level of the data store that would be updated via TexSubImage2D would be the 
		/// sum of level and the value of GL_TEXTURE_VIEW_MIN_LEVEL.
		/// <para>
		/// The following errors can be generated:
		...
		///   GL_TEXTURE_CUBE_MAP_ARRAY and numlayers is not an integer multiple of 6.
		...
		/// and severity form a filter to select messages from the pool of potential messages generated by the GL.
		...
		/// the GL API, the window system, the shader compiler, third party tools or libraries, explicitly by the application or by 
		/// some other source, respectively. It may also take the value GL_DONT_CARE. If source is not GL_DONT_CARE then only 
		...
		/// deprecated features, triggering of undefined behavior, portability issues, performance notifications, markers, group 
		/// push and pop events, and other types of messages, respectively. It may also take the value GL_DONT_CARE. If type is not 
		...
		/// count is zero then the value of ids is ignored. Otherwise, only messages appearing in this list are selected. In this 
		...
		/// be used to classify the message and must be GL_DEBUG_SOURCE_APPLICATION or GL_DEBUG_SOURCE_THIRD_PARTY. All other 
		/// sources are reserved for use by the GL implementation. type indicates the type of the message to be inserted and may be 
		/// one of GL_DEBUG_TYPE_ERROR, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR, GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR, 
		/// GL_DEBUG_TYPE_PORTABILITY, GL_DEBUG_TYPE_PERFORMANCE, GL_DEBUG_TYPE_MARKER, GL_DEBUG_TYPE_PUSH_GROUP, 
		/// GL_DEBUG_TYPE_POP_GROUP, or GL_DEBUG_TYPE_OTHER. severity indicates the severity of the message and may be 
		/// GL_DEBUG_SEVERITY_LOW, GL_DEBUG_SEVERITY_MEDIUM, GL_DEBUG_SEVERITY_HIGH or GL_DEBUG_SEVERITY_NOTIFICATION. id is 
		/// available for application defined use and may be any value. This value will be recorded and used to identify the 
		/// message.
		/// length contains a count of the characters in the character array whose address is given in message. If length is 
		/// negative then message is treated as a null-terminated string. The length of the message, whether specified explicitly or 
		...
		/// associated with the message, and length set to the length of debug message whose character string is in the array 
		/// pointed to by messageuserParam will be set to the value passed in the userParam parameter to the most recent call to 
		...
		/// identifier of each message is written into up to count elements of the array. If severities is not NULL then the 
		/// severity of each message is written into up to count elements of the array. If lengths is not NULL then the length of 
		...
		/// be concatenated onto the array starting at the first element of messageLog. bufSize specifies the size of the array 
		/// messageLog. If a message will not fit into the remaining space in messageLog then the function terminates and returns 
		...
		/// messageLog to retrieve the first message in the queue. If messageLog is NULL then no messages are written and the value 
		/// of bufSize is ignored.
		/// <para>
		/// The following errors can be generated:
		...
		/// it is implied that message contains a null terminated string. The message has the specified source and id, the 
		/// typeGL_DEBUG_TYPE_PUSH_GROUP, and severityGL_DEBUG_SEVERITY_NOTIFICATION. The GL will put a new debug group on top of 
		/// the debug group stack which inherits the control of the volume of debug output of the debug group previously residing on 
		/// the top of the debug group stack. Because debug groups are strictly hierarchical, any additional control of the debug 
		/// output volume will only apply within the active debug group and the debug groups pushed on top of the active debug 
		/// group.
		/// <para>
		/// The following errors can be generated:
		...
		/// message describing its cause based on the message string, the source source, and an ID id submitted to the corresponding 
		/// glPushDebugGroup command. GL_DEBUG_TYPE_PUSH_GROUP and GL_DEBUG_TYPE_POP_GROUP share a single namespace for message id. 
		/// severity has the value GL_DEBUG_SEVERITY_NOTIFICATION. The type has the value GL_DEBUG_TYPE_POP_GROUP. Popping a debug 
		...
		/// identifier must be one of GL_BUFFER, GL_SHADER, GL_PROGRAM, GL_VERTEX_ARRAY, GL_QUERY, GL_PROGRAM_PIPELINE, 
		/// GL_TRANSFORM_FEEDBACK, GL_SAMPLER, GL_TEXTURE, GL_RENDERBUFFER, GL_FRAMEBUFFER, to indicate the namespace containing the 
		/// names of buffers, shaders, programs, vertex array objects, query objects, program pipelines, transform feedback objects, 
		...
		/// in the array identified by label. length contains the address of a variable which will receive the the number of 
		/// characters in the object label. If length is NULL, then it is ignored and no data is written. Likewise, if label is 
