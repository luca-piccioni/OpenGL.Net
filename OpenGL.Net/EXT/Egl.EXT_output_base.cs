...
namespace OpenGL
{
	public partial class Egl
	{
		...
		public static IntPtr GetOutputLayersEXT(IntPtr dpy, IntPtr[] attrib_list, [Out] IntPtr[] layers, int max_layers, [Out] int[] num_layers)
		{
			...
			unsafe {
				...
				fixed (int* p_num_layers = num_layers)
				{
					...
					LogFunction("eglGetOutputLayersEXT(0x{0}, {1}, {2}, {3}, {4}) = {5}", dpy.ToString("X8"), attrib_list, layers, max_layers, num_layers, retValue.ToString("X8"));
				}
			}
		...
		public static IntPtr GetOutputPortsEXT(IntPtr dpy, IntPtr[] attrib_list, [Out] IntPtr[] ports, int max_ports, [Out] int[] num_ports)
		{
			...
			unsafe {
				...
				fixed (int* p_num_ports = num_ports)
				{
					...
					LogFunction("eglGetOutputPortsEXT(0x{0}, {1}, {2}, {3}, {4}) = {5}", dpy.ToString("X8"), attrib_list, ports, max_ports, num_ports, retValue.ToString("X8"));
				}
			}
			DebugCheckErrors(retValue);
		...
		public static IntPtr QueryOutputLayerAttribEXT(IntPtr dpy, IntPtr layer, int attribute, IntPtr[] value)
		{
			...
			unsafe {
				fixed (IntPtr* p_value = value)
				{
					...
					LogFunction("eglQueryOutputLayerAttribEXT(0x{0}, 0x{1}, {2}, {3}) = {4}", dpy.ToString("X8"), layer.ToString("X8"), attribute, value, retValue.ToString("X8"));
				}
			}
			DebugCheckErrors(retValue);
		...
		public static string QueryOutputLayerStringEXT(IntPtr dpy, IntPtr layer, int name)
		{
			...
			LogFunction("eglQueryOutputLayerStringEXT(0x{0}, 0x{1}, {2}) = {3}", dpy.ToString("X8"), layer.ToString("X8"), name, retValue);
			DebugCheckErrors(retValue);
		...
		public static IntPtr QueryOutputPortAttribEXT(IntPtr dpy, IntPtr port, int attribute, IntPtr[] value)
		{
			...
			unsafe {
				fixed (IntPtr* p_value = value)
				{
					...
					LogFunction("eglQueryOutputPortAttribEXT(0x{0}, 0x{1}, {2}, {3}) = {4}", dpy.ToString("X8"), port.ToString("X8"), attribute, value, retValue.ToString("X8"));
				}
			}
			DebugCheckErrors(retValue);
		...
		public static string QueryOutputPortStringEXT(IntPtr dpy, IntPtr port, int name)
		{
			...
			LogFunction("eglQueryOutputPortStringEXT(0x{0}, 0x{1}, {2}) = {3}", dpy.ToString("X8"), port.ToString("X8"), name, retValue);
			DebugCheckErrors(retValue);
