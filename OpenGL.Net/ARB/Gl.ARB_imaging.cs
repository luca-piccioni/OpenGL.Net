...
namespace OpenGL
{
	public partial class Gl
	{
		...
		/// <seecref="Gl.INTENSITY12"/>, <see cref="Gl.INTENSITY16"/>, <see cref="Gl.R3_G3_B2"/>, <see cref="Gl.RGB"/>, <see 
		...
		/// givena particular set of parameters, or to load the contents of a color lookup table. Use the targets <see 
		...
		/// acolor table is specified, <paramref name="data"/> is treated as a byte offset into the buffer object's data store. 
		...
		/// <paramrefname="data"/> is extracted from memory and processed just as if Gl.DrawPixels were called, but processing stops 
		/// afterthe final expansion to RGBA is completed. 
		/// The four scale parameters and the four bias parameters that are defined for the table are then used to scale and bias 
		/// theR, G, B, and A components of each pixel. (Use <see cref="Gl.ColorTableParameter"/> to set these scale and bias 
		/// parameters.)
		/// Next, the R, G, B, and A values are clamped to the range 01. Each pixel is then converted to the internal format 
		/// specifiedby <paramref name="internalformat"/>. This conversion simply maps the component values of the pixel (R, G, B, 
		/// andA) to the values included in the internal format (red, green, blue, alpha, luminance, and intensity). The mapping is 
		/// asfollows: 
		///  
		/// Finally, the red, green, blue, alpha, luminance, and/or intensity components of the resulting pixels are stored in the 
		/// colortable. They form a one-dimensional table with indices in the range 0width-1. 
		/// If <paramref name="target"/> is <see cref="Gl.PROXY_*"/>, <see cref="Gl.ColorTable"/> recomputes and stores the values 
		/// ofthe proxy color table's state variables <see cref="Gl.COLOR_TABLE_FORMAT"/>, <see cref="Gl.COLOR_TABLE_WIDTH"/>, <see 
		...
		/// specifiedcolor table is too large to be supported, then all the proxy state variables listed above are set to zero. 
		/// Otherwise,the color table could be supported by <see cref="Gl.ColorTable"/> using the corresponding non-proxy target, 
		/// andthe proxy state variables are set as if that target were being defined. 
		...
		/// ofeach RGBA pixel group, based on the internal format of the table. 
		/// Each pixel group has color components (R, G, B, A) that are in the range 0.01.0. The color components are rescaled to 
		/// thesize of the color lookup table to form an index. Then a subset of the components based on the internal format of the 
		/// tableare replaced by the table entry selected by that index. If the color components and contents of the table are 
		...
		/// mappedby the color lookup table before being passed to the convolution operation. The colors resulting from the 
		/// convolutionoperation are modified by the post convolution color lookup table when <see 
		...
		/// operationare mapped by the post color matrix color lookup table before being used by the histogram operation. 
		///  
		/// <para>
		...
		/// whenthey are loaded into a color table. <paramref name="target"/> indicates which color table the scale and bias terms 
		/// applyto; it must be set to <see cref="Gl.COLOR_TABLE"/>, <see cref="Gl.POST_CONVOLUTION_COLOR_TABLE"/>, or <see 
		/// cref="Gl.POST_COLOR_MATRIX_COLOR_TABLE"/>.
		...
		/// <seecref="Gl.LUMINANCE"/>, <see cref="Gl.LUMINANCE4"/>, <see cref="Gl.LUMINANCE8"/>, <see cref="Gl.LUMINANCE12"/>, <see 
		...
		/// thanfrom main memory, as is the case for Gl.ColorTable). 
		/// The screen-aligned pixel rectangle with lower-left corner at (<paramref name="x"/>,\ <paramref name="y"/>) having width 
		/// <paramrefname="width"/> and height 1 is loaded into the color table. If any pixels within this region are outside the 
		/// windowthat is associated with the GL context, the values obtained for those pixels are undefined. 
		/// The pixels in the rectangle are processed just as if Gl.ReadPixels were called, with <paramref name="internalformat"/> 
		/// setto RGBA, but processing stops after the final conversion to RGBA. 
		/// The four scale parameters and the four bias parameters that are defined for the table are then used to scale and bias 
		/// theR, G, B, and A components of each pixel. The scale and bias parameters are set by calling Gl.ColorTableParameter. 
		/// Next, the R, G, B, and A values are clamped to the range 01. Each pixel is then converted to the internal format 
		/// specifiedby <paramref name="internalformat"/>. This conversion simply maps the component values of the pixel (R, G, B, 
		/// andA) to the values included in the internal format (red, green, blue, alpha, luminance, and intensity). The mapping is 
		/// asfollows: 
		...
		/// <paramrefname="target"/>. No pixel transfer operations are performed, but pixel storage modes that are applicable to 
		/// Gl.ReadPixelsare performed. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_PACK_BUFFER"/> target (see Gl.BindBuffer) while a 
		/// histogramtable is requested, <paramref name="table"/> is treated as a byte offset into the buffer object's data store. 
		/// Color components that are requested in the specified <paramref name="format"/>, but which are not included in the 
		/// internalformat of the color lookup table, are returned as zero. The assignments of internal color components to the 
		/// componentsrequested by <paramref name="format"/> are 
		///  
		/// <para>
		/// The following errors can be generated:
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="target"/> is not one of the allowable values. 
		...
		///   requiredwould exceed the data store size. 
		/// - <see cref="Gl.INVALID_OPERATION"/> is generated if a non-zero buffer object name is bound to the <see 
		///   cref="Gl.PIXEL_PACK_BUFFER"/>target and <paramref name="table"/> is not evenly divisible into the number of bytes needed 
		...
		/// cref="Gl.POST_CONVOLUTION_COLOR_TABLE"/>,or <see cref="Gl.POST_COLOR_MATRIX_COLOR_TABLE"/> and <paramref name="params"/> 
		/// pointsto an array of four elements, which receive the scale or bias factors for red, green, blue, and alpha, in that 
		/// order.
		/// <see cref="Gl.GetColorTableParameter"/> can also be used to retrieve the format and size parameters for a color table. 
		/// Forthese queries, set <paramref name="target"/> to either the color table target or the proxy color table target. The 
		/// formatand size parameters are set by Gl.ColorTable. 
		/// The following table lists the format and size parameters that may be queried. For each symbolic constant listed below 
		/// for<paramref name="pname"/>, <paramref name="params"/> must point to an array of the given length and receive the values 
		/// indicated.
		///  
		///  
		/// <para>
		...
		/// <paramrefname="start"/> to start+count-1, inclusive. This region may not include any entries outside the range of the 
		/// colortable as it was originally specified. It is not an error to specify a subtexture with width of 0, but such a 
		/// specificationhas no effect. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_UNPACK_BUFFER"/> target (see Gl.BindBuffer) while 
		/// aportion of a color table is respecified, <paramref name="data"/> is treated as a byte offset into the buffer object's 
		/// datastore. 
		/// <para>
		/// The following errors can be generated:
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="target"/> is not one of the allowable values. 
		...
		/// wasoriginally specified. It is not an error to specify a subtexture with width of 0, but such a specification has no 
		/// effect.
		/// <para>
		/// The following errors can be generated:
		...
		/// name="data"/>is extracted from memory and processed just as if Gl.DrawPixels were called, but processing stops after the 
		/// finalexpansion to RGBA is completed. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_UNPACK_BUFFER"/> target (see Gl.BindBuffer) while 
		/// aconvolution filter is specified, <paramref name="data"/> is treated as a byte offset into the buffer object's data 
		/// store.
		/// The R, G, B, and A components of each pixel are next scaled by the four 1D <see cref="Gl.CONVOLUTION_FILTER_SCALE"/> 
		/// parametersand biased by the four 1D <see cref="Gl.CONVOLUTION_FILTER_BIAS"/> parameters. (The scale and bias parameters 
		/// areset by Gl.ConvolutionParameter using the <see cref="Gl.CONVOLUTION_1D"/> target and the names <see 
		/// cref="Gl.CONVOLUTION_FILTER_SCALE"/>and <see cref="Gl.CONVOLUTION_FILTER_BIAS"/>. The parameters themselves are vectors 
		/// offour values that are applied to red, green, blue, and alpha, in that order.) The R, G, B, and A values are not clamped 
		...
		/// simplymaps the component values of the pixel (R, G, B, and A) to the values included in the internal format (red, green, 
		...
		/// ratherthan integer format. They form a one-dimensional filter kernel image indexed with coordinate i such that i starts 
		/// at0 and increases from left to right. Kernel location i is derived from the ith pixel, counting from 0. 
		...
		/// parametersare set by Gl.PixelTransfer. 
		/// <para>
		/// The following errors can be generated:
		...
		///   supportedvalue. This value may be queried with Gl.GetConvolutionParameter using target <see cref="Gl.CONVOLUTION_1D"/> 
		///   andname <see cref="Gl.MAX_CONVOLUTION_WIDTH"/>. 
		/// - <see cref="Gl.INVALID_OPERATION"/> is generated if <paramref name="format"/> is one of <see 
		///   cref="Gl.UNSIGNED_BYTE_3_3_2"/>,<see cref="Gl.UNSIGNED_BYTE_2_3_3_REV"/>, <see cref="Gl.UNSIGNED_SHORT_5_6_5"/>, or <see 
		///   cref="Gl.UNSIGNED_SHORT_5_6_5_REV"/>and <paramref name="type"/> is not <see cref="Gl.RGB"/>. 
		/// - <see cref="Gl.INVALID_OPERATION"/> is generated if <paramref name="format"/> is one of <see 
		///   cref="Gl.UNSIGNED_SHORT_4_4_4_4"/>,<see cref="Gl.UNSIGNED_SHORT_4_4_4_4_REV"/>, <see cref="Gl.UNSIGNED_SHORT_5_5_5_1"/>, 
		///   <seecref="Gl.UNSIGNED_SHORT_1_5_5_5_REV"/>, <see cref="Gl.UNSIGNED_INT_8_8_8_8"/>, <see 
		///   cref="Gl.UNSIGNED_INT_8_8_8_8_REV"/>,<see cref="Gl.UNSIGNED_INT_10_10_10_2"/>, or <see 
		...
		/// butprocessing stops after the final expansion to RGBA is completed. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_UNPACK_BUFFER"/> target (see Gl.BindBuffer) while 
		/// aconvolution filter is specified, <paramref name="data"/> is treated as a byte offset into the buffer object's data 
		/// store.
		/// The R, G, B, and A components of each pixel are next scaled by the four 2D <see cref="Gl.CONVOLUTION_FILTER_SCALE"/> 
		/// parametersand biased by the four 2D <see cref="Gl.CONVOLUTION_FILTER_BIAS"/> parameters. (The scale and bias parameters 
		/// areset by Gl.ConvolutionParameter using the <see cref="Gl.CONVOLUTION_2D"/> target and the names <see 
		/// cref="Gl.CONVOLUTION_FILTER_SCALE"/>and <see cref="Gl.CONVOLUTION_FILTER_BIAS"/>. The parameters themselves are vectors 
		/// offour values that are applied to red, green, blue, and alpha, in that order.) The R, G, B, and A values are not clamped 
		/// to[0,1] at any time during this process. 
		/// Each pixel is then converted to the internal format specified by <paramref name="internalformat"/>. This conversion 
		...
		///  
		/// The red, green, blue, alpha, luminance, and/or intensity components of the resulting pixels are stored in floating-point 
		/// ratherthan integer format. They form a two-dimensional filter kernel image indexed with coordinates i and j such that i 
		/// startsat zero and increases from left to right, and j starts at zero and increases from bottom to top. Kernel location 
		/// i,jis derived from the Nth pixel, where N is i+j*<paramref name="width"/>. 
		/// Note that after a convolution is performed, the resulting color components are also scaled by their corresponding <see 
		/// cref="Gl.POST_CONVOLUTION_c_SCALE"/>parameters and biased by their corresponding <see 
		/// cref="Gl.POST_CONVOLUTION_c_BIAS"/>parameters (where c takes on the values RED, GREEN, BLUE, and ALPHA). These 
		/// parametersare set by Gl.PixelTransfer. 
		...
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="type"/> is not one of the allowable values. 
		/// - <see cref="Gl.INVALID_VALUE"/> is generated if <paramref name="width"/> is less than zero or greater than the maximum 
		///   supportedvalue. This value may be queried with Gl.GetConvolutionParameter using target <see cref="Gl.CONVOLUTION_2D"/> 
		///   andname <see cref="Gl.MAX_CONVOLUTION_WIDTH"/>. 
		/// - <see cref="Gl.INVALID_VALUE"/> is generated if <paramref name="height"/> is less than zero or greater than the maximum 
		///   supportedvalue. This value may be queried with Gl.GetConvolutionParameter using target <see cref="Gl.CONVOLUTION_2D"/> 
		///   andname <see cref="Gl.MAX_CONVOLUTION_HEIGHT"/>. 
		...
		/// fourvalues to be applied to red, green, blue, and alpha values, respectively. The initial value for <see 
		...
		///   <paramrefname="params"/> is not one of <see cref="Gl.REDUCE"/>, <see cref="Gl.CONSTANT_BORDER"/>, or <see 
		...
		/// <paramrefname="width"/> and height 1 is used to define the convolution filter. If any pixels within this region are 
		/// outsidethe window that is associated with the GL context, the values obtained for those pixels are undefined. 
		/// The pixels in the rectangle are processed exactly as if Gl.ReadPixels had been called with format set to RGBA, but the 
		/// processstops just before final conversion. The R, G, B, and A components of each pixel are next scaled by the four 1D 
		/// <seecref="Gl.CONVOLUTION_FILTER_SCALE"/> parameters and biased by the four 1D <see cref="Gl.CONVOLUTION_FILTER_BIAS"/> 
		/// parameters.(The scale and bias parameters are set by Gl.ConvolutionParameter using the <see cref="Gl.CONVOLUTION_1D"/> 
		/// targetand the names <see cref="Gl.CONVOLUTION_FILTER_SCALE"/> and <see cref="Gl.CONVOLUTION_FILTER_BIAS"/>. The 
		/// parametersthemselves are vectors of four values that are applied to red, green, blue, and alpha, in that order.) The R, 
		...
		/// The screen-aligned pixel rectangle with lower-left corner at (<paramref name="x"/>,\ <paramref name="y"/>), width 
		/// <paramrefname="width"/> and height <paramref name="height"/> is used to define the convolution filter. If any pixels 
		/// withinthis region are outside the window that is associated with the GL context, the values obtained for those pixels 
		/// areundefined. 
		/// The pixels in the rectangle are processed exactly as if Gl.ReadPixels had been called with format set to RGBA, but the 
		/// processstops just before final conversion. The R, G, B, and A components of each pixel are next scaled by the four 2D 
		/// <seecref="Gl.CONVOLUTION_FILTER_SCALE"/> parameters and biased by the four 2D <see cref="Gl.CONVOLUTION_FILTER_BIAS"/> 
		...
		/// screencoordinates correspond to lower j filter image coordinates. 
		/// Note that after a convolution is performed, the resulting color components are also scaled by their corresponding <see 
		/// cref="Gl.POST_CONVOLUTION_c_SCALE"/>parameters and biased by their corresponding <see 
		/// cref="Gl.POST_CONVOLUTION_c_BIAS"/>parameters (where c takes on the values RED, GREEN, BLUE, and ALPHA). These 
		/// parametersare set by Gl.PixelTransfer. 
		...
		/// <seecref="Gl.BITMAP"/>, <see cref="Gl.UNSIGNED_SHORT"/>, <see cref="Gl.SHORT"/>, <see cref="Gl.UNSIGNED_INT"/>, <see 
		/// cref="Gl.INT"/>,<see cref="Gl.FLOAT"/>, <see cref="Gl.UNSIGNED_BYTE_3_3_2"/>, <see cref="Gl.UNSIGNED_BYTE_2_3_3_REV"/>, 
		/// <seecref="Gl.UNSIGNED_SHORT_5_6_5"/>, <see cref="Gl.UNSIGNED_SHORT_5_6_5_REV"/>, <see 
		/// cref="Gl.UNSIGNED_SHORT_4_4_4_4"/>,<see cref="Gl.UNSIGNED_SHORT_4_4_4_4_REV"/>, <see cref="Gl.UNSIGNED_SHORT_5_5_5_1"/>, 
		/// <seecref="Gl.UNSIGNED_SHORT_1_5_5_5_REV"/>, <see cref="Gl.UNSIGNED_INT_8_8_8_8"/>, <see 
		...
		/// two-dimensionalimage is placed in <paramref name="image"/> according to the specifications in <paramref name="format"/> 
		/// and<paramref name="type"/>. No pixel transfer operations are performed on this image, but the relevant pixel storage 
		/// modesare applied. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_PACK_BUFFER"/> target (see Gl.BindBuffer) while a 
		/// convolutionfilter is requested, <paramref name="image"/> is treated as a byte offset into the buffer object's data 
		/// store.
		/// Color components that are present in <paramref name="format"/> but not included in the internal format of the filter are 
		/// returnedas zero. The assignments of internal color components to the components of <paramref name="format"/> are as 
		/// follows.
		///  
		/// <para>
		/// The following errors can be generated:
		...
		///   requiredwould exceed the data store size. 
		/// - <see cref="Gl.INVALID_OPERATION"/> is generated if a non-zero buffer object name is bound to the <see 
		///   cref="Gl.PIXEL_PACK_BUFFER"/>target and <paramref name="image"/> is not evenly divisible into the number of bytes needed 
		///   tostore in memory a datum indicated by <paramref name="type"/>. 
		...
		/// convolutionfilter is queried. <paramref name="pname"/> determines which parameter is returned: 
		/// <para>
		/// The following errors can be generated:
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="target"/> is not one of the allowable values. 
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="pname"/> is not one of the allowable values. 
		...
		/// convolutionfilter. The row image is placed in <paramref name="row"/> and the column image is placed in <paramref 
		/// name="column"/>according to the specifications in <paramref name="format"/> and <paramref name="type"/>. (In the current 
		/// implementation,<paramref name="span"/> is not affected in any way.) No pixel transfer operations are performed on the 
		/// images,but the relevant pixel storage modes are applied. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_PACK_BUFFER"/> target (see Gl.BindBuffer) while a 
		/// separableconvolution filter is requested, <paramref name="row"/>, <paramref name="column"/>, and <paramref name="span"/> 
		/// aretreated as a byte offset into the buffer object's data store. 
		/// Color components that are present in <paramref name="format"/> but not included in the internal format of the filters 
		/// arereturned as zero. The assignments of internal color components to the components of <paramref name="format"/> are as 
		/// follows:
		///  
		///  
		/// <para>
		...
		///   requiredwould exceed the data store size. 
		/// - <see cref="Gl.INVALID_OPERATION"/> is generated if a non-zero buffer object name is bound to the <see 
		///   cref="Gl.PIXEL_PACK_BUFFER"/>target and <paramref name="row"/> or <paramref name="column"/> is not evenly divisible into 
		///   thenumber of bytes needed to store in memory a datum indicated by <paramref name="type"/>. 
		...
		/// cref="Gl.UNSIGNED_SHORT_1_5_5_5_REV"/>,<see cref="Gl.UNSIGNED_INT_8_8_8_8"/>, <see cref="Gl.UNSIGNED_INT_8_8_8_8_REV"/>, 
		/// <seecref="Gl.UNSIGNED_INT_10_10_10_2"/>, and <see cref="Gl.UNSIGNED_INT_2_10_10_10_REV"/> are accepted. 
		/// </param>
		/// <param name="row">
		...
		/// expansionto RGBA is completed. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_UNPACK_BUFFER"/> target (see Gl.BindBuffer) while 
		/// aconvolution filter is specified, <paramref name="row"/> and <paramref name="column"/> are treated as byte offsets into 
		/// thebuffer object's data store. 
		...
		/// cref="Gl.CONVOLUTION_FILTER_BIAS"/>.The parameters themselves are vectors of four values that are applied to red, green, 
		/// blue,and alpha, in that order.) The R, G, B, and A values are not clamped to [0,1] at any time during this process. 
		/// Each pixel is then converted to the internal format specified by <paramref name="internalformat"/>. This conversion 
		/// simplymaps the component values of the pixel (R, G, B, and A) to the values included in the internal format (red, green, 
		/// blue,alpha, luminance, and intensity). The mapping is as follows: 
		/// The red, green, blue, alpha, luminance, and/or intensity components of the resulting pixels are stored in floating-point 
		/// ratherthan integer format. They form two one-dimensional filter kernel images. The row image is indexed by coordinate i 
		/// startingat zero and increasing from left to right. Each location in the row image is derived from element i of <paramref 
		/// name="row"/>.The column image is indexed by coordinate j starting at zero and increasing from bottom to top. Each 
		/// locationin the column image is derived from element j of <paramref name="column"/>. 
		/// Note that after a convolution is performed, the resulting color components are also scaled by their corresponding <see 
		/// cref="Gl.POST_CONVOLUTION_c_SCALE"/>parameters and biased by their corresponding <see 
		/// cref="Gl.POST_CONVOLUTION_c_BIAS"/>parameters (where c takes on the values RED, GREEN, BLUE, and ALPHA). These 
		/// parametersare set by Gl.PixelTransfer. 
		...
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="type"/> is not one of the allowable values. 
		/// - <see cref="Gl.INVALID_VALUE"/> is generated if <paramref name="width"/> is less than zero or greater than the maximum 
		///   supportedvalue. This value may be queried with Gl.GetConvolutionParameter using target <see cref="Gl.SEPARABLE_2D"/> and 
		///   name<see cref="Gl.MAX_CONVOLUTION_WIDTH"/>. 
		/// - <see cref="Gl.INVALID_VALUE"/> is generated if <paramref name="height"/> is less than zero or greater than the maximum 
		///   supportedvalue. This value may be queried with Gl.GetConvolutionParameter using target <see cref="Gl.SEPARABLE_2D"/> and 
		///   name<see cref="Gl.MAX_CONVOLUTION_HEIGHT"/>. 
		/// - <see cref="Gl.INVALID_OPERATION"/> is generated if <paramref name="height"/> is one of <see 
		///   cref="Gl.UNSIGNED_BYTE_3_3_2"/>,<see cref="Gl.UNSIGNED_BYTE_2_3_3_REV"/>, <see cref="Gl.UNSIGNED_SHORT_5_6_5"/>, or <see 
		///   cref="Gl.UNSIGNED_SHORT_5_6_5_REV"/>and <paramref name="format"/> is not <see cref="Gl.RGB"/>. 
		/// - <see cref="Gl.INVALID_OPERATION"/> is generated if <paramref name="height"/> is one of <see 
		...
		///   intothe number of bytes needed to store in memory a datum indicated by <paramref name="type"/>. 
		...
		/// imagesare honored. 
		/// If a non-zero named buffer object is bound to the <see cref="Gl.PIXEL_PACK_BUFFER"/> target (see Gl.BindBuffer) while a 
		/// histogramtable is requested, <paramref name="values"/> is treated as a byte offset into the buffer object's data store. 
		/// Color components that are requested in the specified <paramref name="format"/>, but which are not included in the 
		/// internalformat of the histogram, are returned as zero. The assignments of internal color components to the components 
		/// requestedby <paramref name="format"/> are: 
		///  
		/// <para>
		/// The following errors can be generated:
		...
		/// histogramstate information may be queried by calling <see cref="Gl.GetHistogramParameter"/> with a <paramref 
		...
		/// forthe <paramref name="pname"/> argument: 
		///  
		///  
		/// <para>
		/// The following errors can be generated:
		...
		/// (Otherentries are unaltered.) If <see cref="Gl.FALSE"/>, the minmax table is unaltered. 
		/// </param>
		/// <param name="format">
		...
		/// ina one-dimensional image of width 2. The first set of return values are the minima, and the second set of return values 
		/// arethe maxima. The format of the return values is determined by <paramref name="format"/>, and their type is determined 
		...
		/// minimumand maximum pixel values are requested, <paramref name="values"/> is treated as a byte offset into the buffer 
		/// object'sdata store. 
		/// No pixel transfer operations are performed on the return values, but pixel storage modes that are applicable to 
		/// one-dimensionalimages are performed. Color components that are requested in the specified <paramref name="format"/>, but 
		/// thatare not included in the internal format of the minmax table, are returned as zero. The assignment of internal color 
		/// componentsto the components requested by <paramref name="format"/> are as follows: 
		///  
		/// If <paramref name="reset"/> is <see cref="Gl.TRUE"/>, the minmax table entries corresponding to the return values are 
		/// resetto their initial values. Minimum and maximum values that are not returned are not modified, even if <paramref 
		...
		/// takeplace. If <see cref="Gl.FALSE"/>, pixels will proceed to the minmax process after histogramming. 
		/// </param>
		/// <remarks>
		/// When <see cref="Gl.HISTOGRAM"/> is enabled, RGBA color components are converted to histogram table indices by clamping 
		/// tothe range [0,1], multiplying by the width of the histogram table, and rounding to the nearest integer. The table 
		/// entriesselected by the RGBA indices are then incremented. (If the internal format of the histogram table includes 
		/// luminance,then the index derived from the R color component determines the luminance table entry to be incremented.) If 
		/// ahistogram table entry is incremented beyond its maximum value, then its value becomes undefined. (This is not an 
		/// error.)
		/// Histogramming is performed only for RGBA pixels (though these may be specified originally as color indices and converted 
		/// toRGBA by index table lookup). Histogramming is enabled with Gl.Enable and disabled with Gl.Disable. 
		/// When <paramref name="target"/> is <see cref="Gl.HISTOGRAM"/>, <see cref="Gl.Histogram"/> redefines the current histogram 
		/// tableto have <paramref name="width"/> entries of the format specified by <paramref name="internalformat"/>. The entries 
		/// areindexed 0 through width-1, and all entries are initialized to zero. The values in the previous histogram table, if 
		/// any,are lost. If <paramref name="sink"/> is <see cref="Gl.TRUE"/>, then pixels are discarded after histogramming; no 
		/// furtherprocessing of the pixels takes place, and no drawing, texture loading, or pixel readback will result. 
		/// When <paramref name="target"/> is <see cref="Gl.PROXY_HISTOGRAM"/>, <see cref="Gl.Histogram"/> computes all state 
		/// informationas if the histogram table were to be redefined, but does not actually define the new table. If the requested 
		/// histogramtable is too large to be supported, then the state information will be set to zero. This provides a way to 
		/// determineif a histogram table with the given parameters can be supported. 
		///  
		/// <para>
		/// The following errors can be generated:
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="target"/> is not one of the allowable values. 
		...
		///   histogramtable specified is too large for the implementation. 
		...
		/// valuesfor each component, which are stored in the two-element minmax table. (The first element stores the minima, and 
		/// thesecond element stores the maxima.) If a pixel component is greater than the corresponding component in the maximum 
		/// element,then the maximum element is updated with the pixel component value. If a pixel component is less than the 
		/// correspondingcomponent in the minimum element, then the minimum element is updated with the pixel component value. (In 
		/// bothcases, if the internal format of the minmax table includes luminance, then the R color component of incoming pixels 
		/// isused for comparison.) The contents of the minmax table may be retrieved at a later time by calling Gl.GetMinmax. The 
		/// minmaxoperation is enabled or disabled by calling Gl.Enable or Gl.Disable, respectively, with an argument of <see 
		/// cref="Gl.MINMAX"/>.
		...
		/// elementis initialized with the largest possible component values. The values in the previous minmax table, if any, are 
		/// lost.If <paramref name="sink"/> is <see cref="Gl.TRUE"/>, then pixels are discarded after minmax; no further processing 
		/// ofthe pixels takes place, and no drawing, texture loading, or pixel readback will result. 
		///  
		/// <para>
		/// The following errors can be generated:
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="target"/> is not one of the allowable values. 
		...
		/// elementreceives the minimum possible component values, and the ``minimum'' element receives the maximum possible 
		/// componentvalues. 
		/// <para>
		/// The following errors can be generated:
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="target"/> is not <see cref="Gl.MINMAX"/>. 
