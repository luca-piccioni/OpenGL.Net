...
namespace OpenGL
{
	public partial class Gl
	{
		...
		/// colorNumberwhen the program is next linked. index may be zero or one to specify that the color be used as either the 
		/// firstor second color input to the blend equation, respectively. 
		/// The bindings specified by glBindFragDataLocationIndexed have no effect until program is next linked. Bindings may be 
		/// specifiedat any time after program has been created. Specifically, they may be specified before shader objects are 
		/// attachedto the program. Therefore, any name may be specified in name, including a name that is never used as a varying 
		/// outvariable in any fragment shader object. Names beginning with gl_ are reserved by the GL. 
		/// If name was bound previously, its assigned binding is replaced with colorNumber and index. name must be a 
		/// null-terminatedstring. index must be less than or equal to one, and colorNumber must be less than the value of 
		/// GL_MAX_DRAW_BUFFERSif index is zero, and less than the value of GL_MAX_DUAL_SOURCE_DRAW_BUFFERS if index is greater than 
		/// orequal to one. 
		/// In addition to the errors generated by glBindFragDataLocationIndexed, the program program will fail to link if: The 
		/// numberof active outputs is greater than the value GL_MAX_DRAW_BUFFERS. More than one varying out variable is bound to 
		/// thesame color number. 
		/// <para>
		...
		/// programwas last linked. If name is not a varying out variable of program, or if an error occurs, -1 will be returned. 
		/// <para>
		/// The following errors can be generated:
		/// - GL_INVALID_OPERATION is generated if program is not the name of a program object. 
		/// </para>
		...
		/// thoughglBindSampler is called with unit set to the unit the sampler is bound to and sampler zero. Unused names in 
		/// samplersare silently ignored, as is the reserved name zero. 
		/// <para>
		/// The following errors can be generated:
		/// - GL_INVALID_VALUE is generated if n is negative. 
		/// </para>
		...
		/// isnot currently the name of a sampler object, or if an error occurs, glIsSampler returns GL_FALSE. 
		...
		/// previouslyreturned from a call to glGenSamplers. unit must be less than the value of 
		...
		/// singlesampler object may be bound to multiple texture units simultaneously. 
		/// <para>
		/// The following errors can be generated:
		...
		///   ifsuch a name has been deleted by a call to glDeleteSamplers. 
		/// </para>
		/// <para>
		/// The associated information is got with the following commands:
		...
		/// thetarget sampler, and must be the name of an existing sampler object, returned from a previous call to glGenSamplers. 
		/// pnameaccepts the same symbols as glSamplerParameter, with the same interpretations: 
		/// <para>
		/// The following errors can be generated:
		...
		/// timeis recorded after all previous commands on the GL client and server state and the framebuffer have been fully 
		/// realized.When the time is recorded, the query result for that object is marked available. glQueryCounter timer queries 
		/// canbe used within a glBeginQuery / glEndQuery block where the target is GL_TIME_ELAPSED and it does not affect the 
		/// resultof that query object. 
		/// <para>
		/// The following errors can be generated:
		...
		/// primitivesin a single draw call. If divisor is zero, the attribute at slot index advances once per vertex. If divisor is 
		/// non-zero,the attribute advances once per divisor instances of the set(s) of vertices being rendered. An attribute is 
		/// referredto as instanced if its GL_VERTEX_ATTRIB_ARRAY_DIVISOR value is non-zero. 
		...
		/// GL_INT_2_10_10_10_REVor GL_UNSIGNED_INT_2_10_10_10_REV, to specify signed or unsigned data, respectively, or 
		/// GL_UNSIGNED_INT_10F_11F_11F_REVto specify floating point data. 
		/// </param>
		...
		/// normalizedvalue must be GL_FALSE. 
		/// </param>
		/// <param name="value">
