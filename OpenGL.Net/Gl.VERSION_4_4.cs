...
namespace OpenGL
{
	public partial class Gl
	{
		...
		/// ofan existing texture. Further, texture may not be the name of a buffer texture, nor may its internal format be 
		/// compressed.
		/// format and type specify the format and type of the source data and are interpreted as they are for glTexImage3D. 
		/// Textureswith a base internal format of GL_DEPTH_COMPONENT, GL_STENCIL_INDEX, or GL_DEPTH_STENCIL require depth 
		...
		/// constantfill value. The elements of data are converted by the GL into the internal format of the texture image (that was 
		/// specifiedwhen the level was defined by any of the glTexImage*, glTexStorage* or glCopyTexImage* commands), and then used 
		/// tofill the specified range of the destination texture level. If data is NULL, then the pointer is ignored and the 
		/// sub-rangeof the texture image is filled with zeros. If texture is a multisample texture, all the samples in a texel are 
		/// clearedto the value specified by data. 
		/// <para>
		/// The following errors can be generated:
		...
		/// bethe name of an existing texture. Further, texture may not be the name of a buffer texture, nor may its internal format 
		/// becompressed. 
		/// Arguments xoffset, yoffset, and zoffset specify the lower left texel coordinates of a width-wide by height-high by 
		/// depth-deeprectangular subregion of the texel array. 
		/// For one-dimensional array textures, yoffset is interpreted as the first layer to be cleared and height is the number of 
		/// layersto clear. For two-dimensional array textures, zoffset is interpreted as the first layer to be cleared and depth is 
		/// thenumber of layers to clear. Cube map textures are treated as an array of six slices in the z-dimension, where the 
		/// valueof zoffset is interpreted as specifying the cube map face for the corresponding layer and depth is the number of 
		/// facesto clear. For cube map array textures, zoffset is the first layer-face to clear, and depth is the number of 
		/// layer-facesto clear. Each layer-face is translated into an array layer and a cube map face as described in the OpenGL 
		/// Specification.
		...
		/// arrayand taking x, y, z, w, h, and d to be the xoffset, yoffset, zoffset, width, height, and depth argument values, any 
		/// ofthe following relationships generates a GL_INVALID_OPERATION error: 
		...
		///   combinationsthereof) specify a region that falls outside the defined texture image array (including border, if any). 
		/// </para>
		/// <para>
		/// The associated information is got with the following commands:
		/// - glGetTexImage, glGetInternalformat 
		...
		/// consecutivebinding points starting from index index of the array of targets specified by target. If buffers is NULL then 
		/// glBindBuffersBaseunbinds any buffers that are currently bound to the referenced binding points. Assuming no errors are 
		...
		/// countconsecutive binding points starting from index index of the array of targets specified by target. offsets specifies 
		/// theaddress of an array containing count starting offsets within the buffers, and sizes specifies the adderess of an 
		/// arrayof count sizes of the ranges. If buffers is NULL then offsets and sizes are ignored and glBindBuffersRange unbinds 
		/// anybuffers that are currently bound to the referenced binding points. Assuming no errors are generated, it is equivalent 
		/// tothe following pseudo-code, which calls glBindBufferRange: 
		/// <para>
		/// The following errors can be generated:
		/// - GL_INVALID_ENUM is generated if target is not GL_ATOMIC_COUNTER_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER, GL_UNIFORM_BUFFER 
		///   orGL_SHADER_STORAGE_BUFFER. 
		...
		///   describedfor those parameters for the specified target. 
		/// </para>
		/// </remarks>
		/// <seealso cref="Gl.GenBuffers"/>
		/// <seealso cref="Gl.DeleteBuffers"/>
		...
		/// specifiesthe number of texture objects whose names are stored in the array textures. That number of texture names are 
		/// readfrom the array and bound to the count consecutive texture units starting from first. The target, or type of texture 
		/// isdeduced from the texture object and each texture is bound to the corresponding target of the texture unit. If the name 
		/// zeroappears in the textures array, any existing binding to any target of the texture unit is reset and the default 
		/// texturefor that target is bound in its place. Any non-zero entry in textures must be the name of an existing texture 
		...
		/// followingpseudo code: 
		/// Each entry in textures will be checked individually and if found to be invalid, the state for that texture unit will not 
		/// bechanged and an error will be generated. However, the state for other texture units referenced by the command will 
		/// stillbe updated. 
		/// <para>
		/// The following errors can be generated:
		...
		/// units.count specifies the number of sampler objects whose names are stored in the array samplers. That number of sampler 
		/// namesis read from the array and bound to the count consecutive sampler units starting from first. 
		/// If the name zero appears in the samplers array, any existing binding to the sampler unit is reset. Any non-zero entry in 
		/// samplersmust be the name of an existing sampler object. When a non-zero entry in samplers is present, that sampler 
		/// objectis bound to the corresponding sampler unit. If samplers is NULL then it is as if an appropriately sized array 
		/// containingonly zeros had been specified. 
		...
		/// bechanged and an error will be generated. However, the state for other sampler units referenced by the command will 
		/// stillbe updated. 
		/// <para>
		/// The following errors can be generated:
		...
		/// units.count specifies the number of texture objects whose names are stored in the array textures. That number of texture 
		/// namesare read from the array and bound to the count consecutive texture units starting from first. If the name zero 
		/// appearsin the textures array, any existing binding to the image unit is reset. Any non-zero entry in textures must be 
		/// thename of an existing texture object. When a non-zero entry in textures is present, the image at level zero is bound, 
		/// thebinding is considered layered, with the first layer set to zero, and the image is bound for read-write access. The 
		/// imageunit format parameter is taken from the internal format of the image at level zero of the texture object. For cube 
		/// maptextures, the internal format of the positive X image of level zero is used. If textures is NULL then it is as if an 
		/// appropriatelysized array containing only zeros had been specified. 
		...
		/// bechanged and an error will be generated. However, the state for other texture image units referenced by the command 
		/// willstill be updated. 
		/// <para>
		/// The following errors can be generated:
