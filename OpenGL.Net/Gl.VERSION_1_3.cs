...
namespace OpenGL
{
	public partial class Gl
	{
		...
		/// implementation supports is implementation dependent, but must be at least 80.
		/// <para>
		...
		/// antialiasing effects. Multisampling transparently antialiases points, lines, polygons, and images if it is enabled.
		/// value is used in constructing a temporary mask used in determining which samples will be used in resolving the final 
		/// fragment color. This mask is bitwise-anded with the coverage mask generated from the multisampling computation. If the 
		/// invert flag is set, the temporary mask is inverted (all bits flipped) and then the bitwise-and is computed.
		...
		/// generated by combining several samples per pixel. Each sample contains color, depth, and stencil information, allowing 
		/// those operations to be performed on each sample.
		/// <para>
		/// The associated information is got with the following commands:
		...
		/// image if <paramref name="target"/> is <see cref="Gl.TEXTURE_3D"/> (see Gl.TexImage3D).
		...
		/// with Gl.TexImage2D using a generic compressed texture format (e.g., <see cref="Gl.COMPRESSED_RGB"/>), the GL selects 
		/// from one of its extensions supporting compressed textures. In order to load the compressed texture image using <see 
		...
		///   internal formats: <see cref="Gl.COMPRESSED_ALPHA"/>, <see cref="Gl.COMPRESSED_LUMINANCE"/>, <see 
		...
		///   internal format as specified in the specific texture compression extension.
		...
		/// <remarks>
		/// Texturing maps a portion of a specified texture image onto each graphical primitive for which texturing is enabled. To 
		/// enable and disable two-dimensional texturing, call Gl.Enable and Gl.Disable with argument <see cref="Gl.TEXTURE_2D"/>. 
		/// To enable and disable texturing using cube-mapped textures, call Gl.Enable and Gl.Disable with argument <see 
		...
		/// image if <paramref name="target"/> is <see cref="Gl.TEXTURE_2D"/> (see Gl.TexImage2D).
		...
		/// <remarks>
		/// Texturing maps a portion of a specified texture image onto each graphical primitive for which texturing is enabled. To 
		/// enable and disable one-dimensional texturing, call Gl.Enable and Gl.Disable with argument <see cref="Gl.TEXTURE_1D"/>.
		/// <see cref="Gl.CompressedTexImage1D"/> loads a previously defined, and retrieved, compressed one-dimensional texture 
		/// image if <paramref name="target"/> is <see cref="Gl.TEXTURE_1D"/> (see Gl.TexImage1D).
		...
		/// Gl.TexImage1D using a generic compressed texture format (e.g., <see cref="Gl.COMPRESSED_RGB"/>) the GL selects from one 
		/// of its extensions supporting compressed textures. In order to load the compressed texture image using <see 
		...
		/// Gl.TexCoordPointer, and enabled or disabled with Gl.EnableClientState or Gl.DisableClientState, respectively, when 
		/// called with a parameter of <see cref="Gl.TEXTURE_COORD_ARRAY"/>.
		/// <para>
		/// The following errors can be generated:
		/// - <see cref="Gl.INVALID_ENUM"/> is generated if <paramref name="texture"/> is not one of <see cref="Gl.TEXTURE"/>i, where 
		///   i ranges from 0 to the value of <see cref="Gl.MAX_TEXTURE_COORDS"/> - 1.
		/// </para>
		/// <para>
		/// The associated information is got with the following commands:
		...
		/// them to st01. Similarly, <see cref="Gl.MultiTexCoord3"/> specifies the texture coordinates as str1, and <see 
		...
		/// matrix mode (see Gl.MatrixMode).
		/// The current matrix, M, defines a transformation of coordinates. For instance, assume M refers to the modelview matrix. 
		/// If v=v⁡0v⁡1v⁡2v⁡3 is the set of object coordinates of a vertex, and <paramref name="m"/> points to an array of 16 
		/// single- or double-precision floating-point values m=m⁡0m⁡1...m⁡15, then the modelview transformation M⁡v does the 
		/// following:
