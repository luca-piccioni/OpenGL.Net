		...
		public static IntPtr QueryNativeDisplayNV(IntPtr dpy, IntPtr[] display_id)
		{
			...
			unsafe {
				fixed (IntPtr* p_display_id = display_id)
				{
					...
					LogFunction("eglQueryNativeDisplayNV(0x{0}, {1}) = {2}", dpy.ToString("X8"), display_id, retValue.ToString("X8"));
				}
			}
		...
		public static IntPtr QueryNativeWindowNV(IntPtr dpy, IntPtr surf, IntPtr[] window)
		{
			...
			unsafe {
				fixed (IntPtr* p_window = window)
				{
					...
					LogFunction("eglQueryNativeWindowNV(0x{0}, 0x{1}, {2}) = {3}", dpy.ToString("X8"), surf.ToString("X8"), window, retValue.ToString("X8"));
				}
			}
			DebugCheckErrors(retValue);
		...
		public static IntPtr QueryNativePixmapNV(IntPtr dpy, IntPtr surf, IntPtr[] pixmap)
		{
			...
			unsafe {
				fixed (IntPtr* p_pixmap = pixmap)
				{
					...
					LogFunction("eglQueryNativePixmapNV(0x{0}, 0x{1}, {2}) = {3}", dpy.ToString("X8"), surf.ToString("X8"), pixmap, retValue.ToString("X8"));
				}
			}
			DebugCheckErrors(retValue);
