...
namespace OpenGL
{
	public partial class Egl
	{
		...
		public static IntPtr CreateFenceSyncNV(IntPtr dpy, uint condition, int[] attrib_list)
		{
			...
			unsafe {
				fixed (int* p_attrib_list = attrib_list)
				{
					...
					CallLog("eglCreateFenceSyncNV({0}, {1}, {2}) = {3}", dpy, condition, attrib_list, retValue);
				}
			}
			DebugCheckErrors();
		...
		public static IntPtr DestroySyncNV(IntPtr sync)
		{
			...
			CallLog("eglDestroySyncNV({0}) = {1}", sync, retValue);
			DebugCheckErrors();
		...
		public static IntPtr FenceNV(IntPtr sync)
		{
			...
			CallLog("eglFenceNV({0}) = {1}", sync, retValue);
			DebugCheckErrors();
		...
		public static int ClientWaitSyncNV(IntPtr sync, int flags, UInt64 timeout)
		{
			...
			CallLog("eglClientWaitSyncNV({0}, {1}, {2}) = {3}", sync, flags, timeout, retValue);
			DebugCheckErrors();
		...
		public static IntPtr SignalSyncNV(IntPtr sync, uint mode)
		{
			...
			CallLog("eglSignalSyncNV({0}, {1}) = {2}", sync, mode, retValue);
			DebugCheckErrors();
		...
		public static IntPtr GetSyncAttribNV(IntPtr sync, int attribute, int[] value)
		{
			...
			unsafe {
				fixed (int* p_value = value)
				{
					...
					CallLog("eglGetSyncAttribNV({0}, {1}, {2}) = {3}", sync, attribute, value, retValue);
				}
			}
			DebugCheckErrors();
