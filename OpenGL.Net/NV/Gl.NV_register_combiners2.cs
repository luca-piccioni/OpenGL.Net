		...
		public static void CombinerStageParameterNV(Int32 stage, Int32 pname, float[] @params)
		{
			unsafe {
				fixed (float* p_params = @params)
				{
					...
					LogFunction("glCombinerStageParameterfvNV({0}, {1}, {2})", LogEnumName(stage), LogEnumName(pname), @params);
				}
			}
		...
		public static void GetCombinerStageParameterNV(Int32 stage, Int32 pname, [Out] float[] @params)
		{
			unsafe {
				fixed (float* p_params = @params)
				{
					...
					LogFunction("glGetCombinerStageParameterfvNV({0}, {1}, {2})", LogEnumName(stage), LogEnumName(pname), @params);
				}
			}
			DebugCheckErrors(null);
		}
